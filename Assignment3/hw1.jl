...
random_vect = rand(10) # a random vector with 10 items
...
function my_sum(xs)
  # simple for loop method to calculate sum
  sum = 0
  for x in xs
    sum += x
  end
...
m = mean(random_vect) # mean of the random vector
...
function demean(xs)
  mean_xs = mean(xs)  # uses the defines mean() for calculating mean
  res_xs = map(x -> x - mean_xs, xs)# maps the elements of the xs vector to its demean (element - mean(elements)) form
  return res_xs
end
...
# ╔═╡ d9ad7ab9-b5f5-49dd-ad81-70c2196be91d
test_vect # test vector is not changed so the function demean() is non-mutating
...
function create_bar()
  xs = zeros(100)
  xs[40:60] .= 1 # uses broadcasting to set the values in ceter 20 elements as 1
  return xs
end
...
url = "https://easydrawingguides.com/wp-content/uploads/2017/03/how-to-draw-a-lion-featured-image-1200.png"
...
function get_reds(image::AbstractMatrix)
  res = map(pixel -> get_red(pixel), image) # maps the pixels to red channel's value
  # can also use
  # res = get_red.(image)
  # which uses broadcasting a collection over a function
  return res
end
...
begin
  #=
  	Here i used the assignment form of the functions
  	=#
...
function mean_color(image)
  # mean_color is mean of the flattened matrix (with [:]) of matrix from get_color(img) function
  mean_reds = get_reds(image)[:] |> mean
  mean_greens = get_greens(image)[:] |> mean
  mean_blues = get_blues(image)[:] |> mean
  return RGB(mean_reds, mean_greens, mean_blues)
end
...
function noisify(x::Number, s)
  #=
  	Here the formula used for calculating a random value between -s to s is:
  	val = start + rand() * (stop - start) 
  			where (start, stop = -s, s) so it becomes 
  	val = -s + rand() * 2s
  	Adding it to x and clamping within [0, 1] is performed at last.
  	=#
  rand_val = -s + 2s * rand() + x
  rand_val = clamp(rand_val, 0, 1)
  return rand_val
end
...
function custom_filter(pixel::AbstractRGB)
  ...
  # Rndom channel selector filter
  val = rand(1:3) # value between 1 and 3
  return RGB(
    val == 1 ? pixel.r : 0,
    val == 2 ? pixel.g : 0,
    val == 3 ? pixel.b : 0,
...
# ╔═╡ 3e2d9633-2df8-492f-bb38-ce62e8ba2f82
function get_pad(kernel)
	return (size(kernel).-1).÷2
end

# ╔═╡ bf04de64-c25b-46b3-a266-7a0ed95ab022
begin
...
# ╔═╡ 3d1c973d-137c-4260-9b1f-ae8f7e83760e
...
# ╔═╡ 6856e763-7449-4ade-9a3b-551d98a46133
function convolution(image, kernel, pad=Pad(:replicate, get_pad(kernel)))
	...
	if mono
		...
		new_image = [reshape(matrix*vec(channels[i, :, :]), size(image)) for i=1:2]
		return colorview(RGB, new_image[1], new_image[2], new_image[3])
	end
end

# ╔═╡ 69cf5fa9-1e63-4db8-b389-e51f88f3aa02
...
# ╔═╡ bf5d2665-c00b-4c49-bf96-917a04c4b1c8
function invert(col)
	return RGB(1-col.r, 1-col.g, 1-col.b)
end
...
philip_inverted = invert.(philip) # broadcast all pixels to invert function

# ╔═╡ 2cd320ee-2bb6-4b8c-9bdf-97eede5df2cf
function decolourize(col)
	v = max(col.r, col.g, col.b)
	return RGB(v, v, v)
end

# ╔═╡ 77a820eb-d53c-48f9-ad46-eaa02c502015
[invert.(philip) decolourize.(philip)]

# ╔═╡ 91e27809-8220-4bc8-a29f-7cc77707aa8d
norm(x) = x/sum(x)

# ╔═╡ 48c5fe3f-9384-4033-9e3f-88b41c8a0743
uniformBlur(n) = centered(norm(ones(2n+1,2n+1)))

# ╔═╡ 755125f8-9659-4d94-b268-b50084a83c56
normalBlur(n) = centered(norm([9exp((-9x^2-9y^2)/2n^2) for x in -n:n, y in -n:n]))

# ╔═╡ d88b60de-6592-4a18-9fdf-07d2c297e787
[imfilter(philip, uniformBlur(5)) imfilter(philip, normalBlur(10))]

# ╔═╡ af39e00f-56ac-452f-a217-1bbb71df24e1
sharpen = centered([0 -1 0; -1 5 -1; 0 -1 0])

# ╔═╡ 5ede08fe-137b-496b-addf-28bee13dbbd1
[imfilter(philip, normalBlur(4)) imfilter(philip, (normalBlur(14), sharpen)) imfilter(philip, (normalBlur(24), sharpen, sharpen))]

# ╔═╡ 6a2a2bc1-3610-427e-90ce-a376dea9c7db
detectEdge = centered([0 -1 0; -1 4 -1; 0 -1 0])

# ╔═╡ 726619cc-464d-4a46-ac1a-b70dd60c7c0a
f(x) = invert(decolourize(x))

# ╔═╡ 56b93339-8560-4d49-9fbb-5006eb3900ff
f.([imfilter(philip, detectEdge) imfilter(philip, (normalBlur(2), detectEdge))])
...
customized_image = custom_filter(cam_image)

# ╔═╡ 52d027d2-d2ae-4718-83dc-5224180fddda
filtered_pad = imfilter(cam_image, kernel, pad, Algorithm.FIR())

# ╔═╡ db00e950-9561-4a5f-b870-65775c4b0e59
begin
	estimate = false
...
# ╔═╡ de35fc01-7cae-4412-b24e-f22a7d51d622
gray_image = Gray.(cam_image)
...
manifest_format = "2.0"
project_hash = "02315203af3e588b2accfe5dcc877c06581f8b01"
